...
class Goats {
    ...
    log(msg, type = 'info') {
        ...
        switch(type) {
            case 'success':
                console.log(`[${timestamp}] [FOREST ARMY] ${msg}`.green);
                break;
            case 'custom':
                console.log(`[${timestamp}] [FOREST ARMY] ${msg}`.magenta);
                break;        
            case 'error':
                console.log(`[${timestamp}] [SATYAVIRKUMARSATYARTHI] ${msg}`.red);
                break;
            case 'warning':
                console.log(`[${timestamp}] [FOREST ARMY] ${msg}`.yellow);
                break;
            default:
                console.log(`[${timestamp}] [FOREST ARMY] ${msg}`.blue);
        }
    }
    ...
    async handleMissions(accessToken) {
        ...
        for (const mission of regular) {
            const result = await this.completeMission(mission, accessToken);
            if (result) {
                this.log(`FORESTARMY-Successfully completed mission ${mission.name} | Reward: ${mission.reward}`, 'success');
            } else {
                this.log(`Failed to complete mission ${mission.name}`, 'error');
            }
            await new Promise(resolve => setTimeout(resolve, 1000));
    ...
    async main() {
        ...
        while (true) {
            for (let i = 0; i < data.length; i++) {
                ...
                console.log(`========== CODE SHARED BY FORESTARMY ${i + 1} | ${firstName} ==========`.blue);
